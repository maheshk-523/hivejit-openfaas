#!/usr/bin/env python3
"""Run sequential Go/OpenFaaS PGO invocations with real pod churn."""
...
def summarize(rows: list[dict[str, Any]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    handler_latencies = [float(row["handler_elapsed_ms"]) for row in ok_rows]
    return {
        "benchmark": args.benchmark,
...
def main() -> int:
    ...
    parser.add_argument("--function", default="go-pgo-redis")
    ...
    parser.add_argument("--profile-iters", type=int, default=0)
    parser.add_argument("--requests", type=int, default=350_000)
    ...
    for invocation in range(1, args.invocations + 1):
        ...
        row = {
            "benchmark": payload.get("benchmark") or args.benchmark,
            "treatment": args.treatment,
            "profile_iters": args.profile_iters,
            ...
            "handler_elapsed_ms": f"{as_float(payload.get('work_ms')):.6f}",
