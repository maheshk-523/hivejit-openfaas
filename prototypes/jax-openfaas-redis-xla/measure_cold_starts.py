#!/usr/bin/env python3
"""Measure first-request latency across fresh OpenFaaS pods."""
...
def list_function_pods(args: argparse.Namespace) -> list[dict[str, Any]]:
    data = kubectl_json(
        args,
        ["get", "pod", "-n", args.namespace, "-l", f"faas_function={args.function}"],
        timeout=30.0,
    )
...
def wait_ready_pod(args: argparse.Namespace, started: float) -> tuple[str, str, float]:
    ...
    while time.time() < deadline:
        ...
        for pod in pods:
            ...
            last_phase = f"{name}:{phase}" if name else phase
        time.sleep(1.0)
    raise TimeoutError(f"timed out waiting for ready pod; last state={last_phase}")
...
def invoke(args: argparse.Namespace, trial: int) -> tuple[int, float, bytes, str]:
    ...
    try:
        ...
        status = 0
        error = str(exc)
    return status, (time.perf_counter() - started) * 1000.0, body, error
...
def flatten_row(
    ...
    return {
        ...
        "first_work": str(bool(payload.get("first_work"))).lower(),
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    http = values("http_latency_ms")
    compile_load = values("compile_or_load_ms")
    restart = values("restart_ms")
    import_ms = values("import_ms")
    ...
    return {
        ...
        "signature": rows[0]["signature"] if rows else "",
        "trials": len(rows),
        "statuses": statuses,
        "http_latency_ms": stats_block(http),
        "compile_or_load_ms": stats_block(compile_load),
        "restart_ms": stats_block(restart),
        "import_ms": stats_block(import_ms),
    }
...
def main() -> int:
    ...
    for trial in range(1, args.trials + 1):
        ...
        status, http_latency_ms, body, error = invoke(args, trial)
        row = flatten_row(args, trial, pod, pod_uid, restart_ms, status, http_latency_ms, body, error)
        rows.append(row)
        print(
            json.dumps(
                {
