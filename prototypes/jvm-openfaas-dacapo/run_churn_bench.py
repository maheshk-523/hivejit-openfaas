#!/usr/bin/env python3
"""Run sequential DaCapo/OpenFaaS invocations with scripted pod churn."""
...
def summarize(rows: list[dict[str, Any]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    handler_latencies = [float(row["handler_elapsed_ms"]) for row in ok_rows if float(row["handler_elapsed_ms"]) > 0]
    return {
        "benchmark": args.benchmark,
        "size": args.size,
...
def main() -> int:
    ...
    parser.add_argument("--function", default="dacapo-jvm")
    parser.add_argument("--namespace", default="openfaas-fn")
    parser.add_argument("--gateway", default="http://127.0.0.1:8080")
    parser.add_argument("--benchmark", choices=["h2", "lusearch", "eclipse", "fop", "jython"], required=True)
    parser.add_argument("--size", default="small")
    parser.add_argument("--iterations", type=int, default=1)
    parser.add_argument("--threads", default="1")
    parser.add_argument("--pre-gc", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--validation", choices=["default", "ignore", "none", "false"], default="none")
